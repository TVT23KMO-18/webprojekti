...
async function getFavoritesByLink(shareableLink) {
    const queryText = 'SELECT * FROM favorites WHERE shareable_link = $1';
    try {
        const { rows } = await pgPool.query(queryText, [shareableLink]);
        return rows;
    } catch (error) {
...
async function storeShareableLink(userid, shareableLink) {
  const queryText = 'UPDATE favorites SET shareable_link = $1 WHERE iduser = $2';
  const values = [shareableLink, userid];
  try {
