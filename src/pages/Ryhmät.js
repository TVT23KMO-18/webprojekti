    ...
    const fetchAllGroups = async () => {
      try {
        if(!user) {
          ...
          const response = await axios.get(`http://localhost:3001/group/groupsbyusername?username=${user.username}`);
          setAllGroups(response.data);
        }     
      } catch (error) {
      ...
      <div className='ryhma-grid-container'>
            ...
            <div className='kolme'>
              ...
              <div className='ryhmanimi-input'>
                <div>
                  <input required placeholder='Ryhmän nimi' value={groupName} onChange={handleInputChange}></input>
                  <input required placeholder='Ryhmän kuvaus' value={groupDescription} onChange={handleInputChangeTwo}></input>
                  <button onClick={ryhmä} disabled={!groupName || !groupDescription}>Luo ryhmä</button>
                </div>
              </div>
            </div>
